...
def create_injector(
    ...
    if payload_type == "eicar":
        ...
        injector_config = {
            'message': config.get('alert_message', 'XSS Code Injection Test')
        }
        return AlertTestInjector(injector_config)
    
    elif payload_type == "keylogger":
        injector_config = {
            'max_log_entries': config.get('keylogger_max_entries', 50)
        }
...
Examples:
  ...
  python -m src.cli --i-have-authorization --arp-spoof --gateway 192.168.1.1
    ...
    parser.add_argument(
        '--custom-file',
        help='Path to custom JavaScript file (required for --payload custom)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='NFQUEUE number (default: 0)'
    )
    
    parser.add_argument(
        '--gateway',
        help='Gateway IP address (required for ARP spoofing)'
    )
    ...
    parser.add_argument(
        '--arp-spoof',
        action='store_true',
        help='Enable ARP spoofing for MITM positioning'
    )
    ...
    parser.add_argument(
        '--subnet',
        help='Subnet to scan in auto-hook mode (e.g., 192.168.1.0/24)'
    )
    ...
    parser.add_argument(
        '--beef-host',
        default='127.0.0.1',
        help='BeEF server host (default: 127.0.0.1)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3000,
        help='BeEF server port (default: 3000)'
    )
    
    parser.add_argument(
        '--beef-api-token',
        help='BeEF REST API token for hook monitoring'
    )
    
    parser.add_argument(
        '--monitor-beef',
        action='store_true',
        help='Enable BeEF hook monitoring'
    )
